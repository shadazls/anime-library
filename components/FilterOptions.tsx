...
const FilterOptions: React.FC<FilterOptionsProps> = ({ onSearch }) => {
    ...
    >([]); // Format des producteurs
    const [licensors, setLicensors] = useState<
        { label: string; value: string }[]
    >([]); // Format des licensors
    ...
    useEffect(() => {
        const fetchProducers = async () => {
            try {
                ...
                const producerOptions = data.map(
                    (producer: { _id: string }) => ({
                        label: producer._id,
                        value: producer._id,
                    })
    ...
    useEffect(() => {
        const fetchLicensors = async () => {
            try {
                ...
                const licensorOptions = data.map(
                    (licensor: { _id: string }) => ({
                        label: licensor._id,
                        value: licensor._id,
                    })
            ...
            {isFiltersVisible && (
                <>
                    ...
                    <AnimeFilter
                        ...
                        options={[
                            {
                                label: 'Apr 3, 1998 to Apr 24, 1999',
                                value: 'Apr 3, 1998 to Apr 24, 1999',
                            },
                            {
                                label: 'Mar 1, 2010 to Mar 30, 2011',
                                value: 'Mar 1, 2010 to Mar 30, 2011',
                            },
                            // Add more options as needed
                        ]}
                    />
